		...
		while( ! isStop && ! client.socket.isClosed())
		{
			try
			{
				...
				if(type.equalsIgnoreCase("聊天信息"))
				{
					...
					textArea.append(chatMessageContentTitle
							+ chatMessageContent);
					
					if(toSomebody.equals(client.username))
					{
						try
						{
							...
							if(status.equalsIgnoreCase("陌生人信息"))
							{
								try
								{
									...
									chatMessageContent = " [ 陌生人信息 ]  " + "你"
											...
											+ "\n";
									client.output
											.writeObject(chatMessageContent);
									client.output.flush();
								}
								catch(Exception e)
								{
...
//									System.out.println(str);
								...
								chatMessageContent = " [ 正常信息 ]  "
										...
										+ " 对 所有人 说 : " + message + "\n";
								sendToAll(chatMessageContent , client);// 向除自己外的所有人发送消息
							}
						}
						else//私聊
						{
